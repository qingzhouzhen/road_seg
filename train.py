...
from RoadData import train_dataloader
...
def train(epo_num=50, show_vgg_params=False):
    ...
    for epo in range(epo_num):
        ...
        for index, (road, road_msk) in enumerate(train_dataloader):
            # road.shape is torch.Size([4, 3, 160, 160])
            # road_msk.shape is torch.Size([4, 2, 160, 160])
            ...
            loss = criterion(output, road_msk)
            loss.backward()
            iter_loss = loss.item()
            ...
            road_msk_np = road_msk.cpu().detach().numpy().copy() # road_msk_np.shape = (4, 2, 160, 160)
            ...
            if np.mod(index, 15) == 0:
                ...
                # vis.images(road_msk_np[:, None, :, :], win='train_label', opts=dict(title='label'))
        ...
        #            print(r'Testing... Open http://localhost:8097/ to see test result.')
                    ...
                    # vis.images(road_msk_np[:, None, :, :], win='test_label', opts=dict(title='label'))
...
if __name__ == "__main__":
    ...
    train(epo_num=1000, show_vgg_params=False)
